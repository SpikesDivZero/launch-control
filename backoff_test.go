...
func TestExpBackoff(t *testing.T) {
	...
	// Can't particularly test jitter behaviors from the public interface, so we'll just approximate it by calling it
	// a bunch of times and seeing that it always returns in range. Works well enough for the public func.
	...
	t.Run("zero min", func(t *testing.T) {
		defer testutil.WantPanic(t, "ExpBackoff: min and max delay must not be zero")
	...
	t.Run("min gt max", func(t *testing.T) {
		defer testutil.WantPanic(t, "ExpBackoff: minDelay must be less than maxDelay")
